...
class vgg16:
    ...
    def __init__(self, lr=0.01, lr_decay=1, mbs=1, pred_mbs=None):
        self.lr = lr
    ...
    def fc_layers(self):
        ...
        with tf.name_scope('fc2') as scope:
            ...
            fc2b = tf.Variable(tf.constant(1.0, shape=[512], dtype=tf.float32),
                                 trainable=True, name='biases')
            fc1_bn = tf.layers_batch_normalization(self.fc1, training=self.is_training)
            fc1_drop = tf.layers_dropout(fc1_bn, training=self.is_training)
        ...
        with tf.name_scope('fc3') as scope:
            fc3w = tf.Variable(tf.truncated_normal([512, 10],
                                                         dtype=tf.float32,
                                                         stddev=1e-1), name='weights')
            fc3b = tf.Variable(tf.constant(1.0, shape=[10], dtype=tf.float32),
                                 trainable=True, name='biases')
            fc2_bn = tf.layers_batch_normalization(self.fc2, training=self.is_training)
            fc2_drop = tf.layers_dropout(fc2_bn, training=self.is_training)
