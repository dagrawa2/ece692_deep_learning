...
class NN:
	...
	def backward(self, Y, a):
		...
		for W,b,a_,delta in zip(self.W, self.b, a[:-1], deltas):
			...
			grad_b = 1/Y.shape[0]*np.sum(delta, axis=0, keepdims=True)
			self.temp_grads.append(np.sqrt(np.sum(grad_b**2)))
			b -= self.eta*grad_b
...
#			print("Epoch ", epoch, " . . . ")
			...
			grads.append( np.mean(np.array(self.temp_grads).reshape((-1, len(self.b)))[:,:-1], axis=0) )
#			print("\t Test accuracy: ", np.round(accs_test[-1], 6))
			if 1+epoch >= early_stopping and np.argmax(accs_test[-early_stopping:])==0: break
		return np.array(accs_test), np.stack(grads, axis=1)
