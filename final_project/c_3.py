...
layers = [Input(input_dim),
	...
	Unfold(name="unfold"),
#	FullyConnected([8*8*64, output_dim], name="fc2")
]

print("Initializing base and deep kernels . . . ")
base_kernel = kernels.NNGP(8*8*64, v_b=1.0, v_w=1.0, depth=1)
...
model.save_params("results/c_3_params.npz")

print("Saving accuracies . . . ")
with open("results/c_3_accs.csv", "w") as fp:
	fp.write("epoch,acc\n")
	for i, a in enumerate(accs):
		fp.write(str(i+1)+","+str(a)+"\n")

print("Saving test predictions . . . ")
with open("results/c_3_preds.csv", "w") as fp:
...
with open("results/c_3_arc.csv", "w") as fp:
	fp.write("rej,acc\n")
