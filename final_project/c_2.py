...
(X_train, Y_train), (X_test, Y_test) = datasets.load_cifar10(n_classes=10, p=0.2, map_into_unit_range=True)

print("Initializing NN layers . . . ")
input_dim = list(X_train.shape)[1:]
output_dim = Y_train.shape[1]
layers = [Input(input_dim),
	Convolution2D([3, 3, 3, 32], activation=tf.nn.relu, name="conv1_1"),
	...
	Unfold(name="unfold"),
#	FullyConnected([8*8*64, 1024], activation=tf.nn.relu, name="fc1"),
...
base_kernel = gpflow.kernels.RBF(8*8*64, variance=1.0, lengthscales=l_0)
...
model.save_params("results/c_2_params.npz")

print("Saving accuracies . . . ")
with open("results/c_2_accs.csv", "w") as fp:
	fp.write("epoch,acc\n")
	for i, a in enumerate(accs):
		fp.write(str(i+1)+","+str(a)+"\n")

print("Saving test predictions . . . ")
with open("results/c_2_preds.csv", "w") as fp:
...
with open("results/c_2_arc.csv", "w") as fp:
	fp.write("rej,acc\n")
