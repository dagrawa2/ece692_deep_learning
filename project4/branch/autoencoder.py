...
class autoencoder:
	...
	def build_encoder(self):
		self.input_shapes_partial = []
		self.input_shape_tensors = []
		self.params = []
		self.param_names = []
		out = tf.identity(self.x)
		for layer in self.encoder_layers:
			self.input_shape_tensors.append(tf.shape(out))
			out = layer(out)
			self.input_shapes_partial.append(layer.input_shape)
	...
	def build_decoder_layers(self):
		...
		for layer in self.encoder_layers:
			...
			if len(layer.get_param_names()) > 0:
				break
		self.decoder_layers = []
		for i, (layer, output_shape) in enumerate(list(zip(self.encoder_layers, self.input_shape_tensors))[::-1]):
	...
	def build_block_autoencoder(self, block):
		self.block_input = tf.placeholder(tf.float32, shape=self.input_shapes_partial[block[0]])
		out = tf.identity(self.block_input)
		for layer in [self.encoder_layers[i] for i in block]:
			out = layer(out)
		self.block_encoded = tf.identity(out)
		for layer in [self.decoder_layers[-1-i] for i in block[::-1]]:
			if layer is not None:
				out = layer(out)
